...
Tree.prototype.transplant = function(u, v, cb) {
    var tree = this;
    v.parent = u.parent;
    if (u.parent === this.nil.id) {
        this.setRedis(v, true, cb);
    } else {
        this.getRedis(u.parent, function(parent) {
            if (u.id === parent.left) {
                parent.left = v.id;
            } else {
                parent.right = v.id;
            ...
            tree.setRedis(parent, false, function() {
                tree.setRedis(v, false, cb);
            });
        });
    }
};

Tree.prototype.remove = function(z) {
};
